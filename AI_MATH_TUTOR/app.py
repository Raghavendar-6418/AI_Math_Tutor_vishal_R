...
def main():
    ...
    st.sidebar.image(img, caption="Input image", width="stretch")
    ...
    # Inga 'user_math_expr' nu oru variable add pannirukom
    user_math_expr = st.text_input("Detected expression", value=math_expr)

    latex = None
    if use_llm:
        with st.spinner("Converting to LaTeX via LLM..."):
            latex = llm_convert_to_latex(user_math_expr, image=to_bytes(pre))
    else:
        latex = user_math_expr.strip()
    ...
    if not latex or not latex.strip():
        st.warning("No expression to parse. Please provide an equation or expression.")
        return
